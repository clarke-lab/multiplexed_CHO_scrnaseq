...
wgcna_boxplot_filtered <- plot_df %>%
  filter(variable %in% c("MEblue", "MEmagenta", "MEyellow")) %>%
  mutate(new=factor(variable,labels=c("MEblue","MEmagenta","MEyellow"),  levels=c("MEblue","MEmagenta","MEyellow"))) %>%
  ggplot(aes(x = orig.ident, y = value, fill = orig.ident)) +
  scale_fill_manual(values = c("#fde725", "#5ec962", "#21918c", "#31688e", "#440154")) +
...
```{r eval=FALSE, fig.width=4, include=FALSE}
library(reshape2)
library(sna)
library(Hmisc)
library(ggnetwork)
