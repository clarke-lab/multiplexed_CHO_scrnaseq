...
package_list <- c(
  ...
  "Seurat", "viridis", "WebGestaltR", "ggpubr", "patchwork", "scater","ggExtra", "SeuratWrappers"
)
...
```
## Cellular scRNA-seq preprocessing
...
#
 ...
 knee_plot_lib1 <- knee_plot(bc_rank1) + labs(title="mRNA library 1") + theme_bw()
...
# knee_plot(bc_rank)
 ...
 knee_plot_lib2 <- knee_plot(bc_rank2) + labs(title="mRNA library 2") + theme_bw()
...
filter_cells <- function(object) {
  ...
  umi_v_feature_plot <- object@meta.data %>%
  ...
  umi_v_feature_plot <- ggMarginal(umi_v_feature_plot, type = "density", fill = "lightgrey")
...
cell_rna_library_1_filter_output$mtdna_plot
...
cell_rna_library_2_filter_output$mtdna_plot
...
#colnames(tagged_cells_seurat_matrix2) <- gsub("-1", "", colnames(tagged_cells_seurat_matrix2))
...
eval_pq_lib2_plot <- eval_pq_lib2 %>%
  ggplot(aes(x=threshold, y=cells, color=SBO_classification.global)) +
           geom_point() +
           geom_line() + 
  geom_vline(xintercept = 0.97, linetype=11, size=0.2) +
  theme_bw() +
...
ggsave(plot = sbo_classification_barplot, filename = paste(results_dir, "Figure 2c.png", sep = ""), 
...
# cho.combined.sct <- cho.combined.sct %>%
#   NormalizeData() %>%
#   FindVariableFeatures() %>%
#   ScaleData(vars.to.regress = c("S.Score", "G2M.Score","nFeature_RNA", "percent.mt", "nCount_RNA"))
...
cho.combined.sct <- SCTransform(
  ...
  assay = "RNA",
  new.assay.name = "SCT",
  vars.to.regress = c("nFeature_RNA"), variable.features.n = 4000
)
...
cho.combined.sct <- SCTransform(
  ...
  assay = "RNA",
  new.assay.name = "SCT",
  vars.to.regress = c("S.Score", "G2M.Score","nFeature_RNA"), variable.features.n = 4000,
  verbose = TRUE
...
# VlnPlot(cell_library_2, group.by = "orig.ident", features = "nFeature_RNA", pt.size = 0.1, ncol = 3) +
#     NoLegend()
```
...
figure_s5
ggsave(plot = figure_s5, filename = paste(results_dir, "Figure S5.png", sep = ""), width = 5, height = 5, device = "png", dpi = 700)
```
...
figure_s6 <- (figure_s6a + plot_spacer() + figure_s6b) + plot_layout(widths =  c(4,0.5,4)) + plot_annotation(tag_levels = 'a')
figure_s6 
ggsave(plot = figure_s6, filename = paste(results_dir, "Figure S6.png", sep = ""), width = 6, height = 2, device = "png", dpi = 700)
```
...
figure_2e
ggsave(plot = figure_2e, filename = paste(results_dir, "Figure 2d.png", sep = ""), width = 5, height = 4, device = "png", dpi = 700)
```
```{r}
...
langevitour(as.data.frame(cho.combined.sct.3d[["umap"]]@cell.embeddings), cho.combined.sct.3d$treatment)
```
...
figure2defg <- figure_2e + marker_gene_umap
ggsave(plot = figure2defg, filename = paste(results_dir, "Figure 2EFG.png", sep = ""), width = 15, height = 4.5, device = "png", dpi = 700)
```
...
figure_s8a <- powerTable$data %>%
  ggplot(aes(x=Power, y=SFT.R.sq)) +
  geom_point(size=0.1) +
  labs(y="SFT fit", x="Power") +
  geom_vline(xintercept = 18, linetype=11, size=0.2, color="red") +
  theme_bw()
  
figure_s8b <- powerTable$data %>%
  ggplot(aes(x=Power, y=median.k.)) +
  geom_point(size=0.1) +
  labs(y="Median connectivity", x="Power") +
  geom_vline(xintercept = 18, linetype=11, size=0.2, color="red") +
  theme_bw()

figure_s8c <- powerTable$data %>%
...
figure_s8d <- powerTable$data %>%
  ggplot(aes(x=Power, y=max.k.)) +
  geom_point(size=0.1) + 
  labs(y="Max connectivity", x="Power") +
  geom_vline(xintercept = 18, linetype=11, size=0.2, color="red") +
  theme_bw() 

figure_s8 <- (figure_s8a +  figure_s8b ) / (figure_s8c  +figure_s8d) +plot_annotation(tag_levels = "a")

ggsave(plot = figure_s8, filename = paste(results_dir, "Figure S8.png", sep = ""), width = 6.5, height = 4, device = "png", dpi = 700)
```
...
wgcna_boxplot_filtered <- plot_df %>%
  filter(variable %in% c("MEturquoise", "MEmagenta", "MEyellow")) %>%
  mutate(new=factor(variable,labels=c("MEturquoise","MEmagenta","MEyellow"),  levels=c("MEturquoise","MEmagenta","MEyellow"))) %>%
  ggplot(aes(x = orig.ident, y = value, fill = orig.ident)) +
  scale_fill_manual(values = c("#fde725", "#5ec962", "#21918c", "#31688e", "#440154")) +
  #geom_violin(draw_quantiles = T, width=0.4) +
  geom_boxplot(notch = T, alpha=0.5) + 
  geom_jitter(size=0.1, alpha=0.3, color="grey") +
  RotatedAxis() +
  ylab("Module Eigengene") +
  xlab("") +
  theme_bw() +
...
saveRDS(pcg_with_loc_ids, file="../data/pcg_with_loc_ids.rds")
...
selected_colors <- unique(geneInfo$Initially.Assigned.Module.Color)
for (color in selected_colors) {
  current_genes <- geneInfo %>%
  ...
  magenta_biomart <- getBM(attributes = c('ensembl_gene_id_version', 'external_gene_name'),
      ...
      values = current_genes$GeneSymbol, 
      mart = ensembl)
  
  write(magenta_biomart$external_gene_name, file = paste0(enrichdir, color, "_genes.txt"))
}
...
blue_module_enrichment <- enrich_result[[2]]$enrichResult[, c(1, 2, 4, 5, 7, 8, 9, 11)] %>%
  mutate(type = "MEblue")

green_module_enrichment <- enrich_result[[4]]$enrichResult[, c(1, 2, 4, 5, 7, 8, 9, 11)] %>%
  mutate(type = "MEgreen")

turquoise_module_enrichment <- enrich_result[[7]]$enrichResult[, c(1, 2, 4, 5, 7, 8, 9, 11)] %>%
  mutate(type = "MEturquoise")


enrichment_plot_data <- bind_rows(turquoise_module_enrichment[1:6, ], blue_module_enrichment[1:6, ], green_module_enrichment[1:6, ])
...
enrichment_plots <- enrichment_plot_data %>%
  ...
  mutate(new=factor(type,labels=c("MEturquoise","MEblue","MEgreen"),  levels=c("MEturquoise","MEblue","MEgreen"))) %>%
  ...
  facet_wrap(~new, scales = "free", ncol = 3) +
  scale_fill_manual(values=c("turquoise","blue", "green")) +
...
ggsave(plot = test, filename = paste(results_dir, "Figure 3d.png", sep = ""), width = 10, height =6.5, device = "png", dpi = 700)
```
...
asap1_umap <- FeaturePlot(cho.combined.sct, features = c("Pxdn-1"))$data %>%
  mutate(gene = "Pxdn.1") %>%
  rename(expression = "Pxdn.1") %>%
  ggplot(aes(x = UMAP_1, y = UMAP_2, color = expression)) +
  geom_point(size = 0.01) +
  scale_color_viridis(option = "turbo", direction = 1) +
  theme_bw() +
