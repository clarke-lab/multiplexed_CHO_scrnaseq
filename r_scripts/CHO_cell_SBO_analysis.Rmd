...
cell_library_1 <- subset(cell_library_1,
  subset = nFeature_RNA > feat.min.lib1  &
    nFeature_RNA < feat.max.lib1  &
    nCount_RNA > count.min.lib1 &
    nCount_RNA < count.max.lib1 &
...
cell_library_2 <- subset(cell_library_2,
  subset = nFeature_RNA > feat.min.lib2  &
    nFeature_RNA < feat.max.lib2  &
    nCount_RNA > count.min.lib2 &
    nCount_RNA < count.max.lib2 &
    percent.mt < mt_threshold
)
...
preprocessing_violin <- bind_rows(cell_library_1_meta_data, cell_library_2_meta_data) %>%
...
preprocessing_violin
ggsave(plot = preprocessing_violin, filename = paste(results_dir, "Figure S1.png", sep = ""), width = 7, height = 5, device = "png", dpi = 700)
```
...
sum(!colnames(sbo_lib_1.data) %in% "TCATGGATCGGAGTGA")

as_tibble(sbo_lib_1.data@meta.data,rownames = "cell_label") %>%
  filter(!cell_label %in% cho_lib1_joint_bcs)
...
```{r}
per_sample_sbo_classifications <- bind_rows(classification_library_1, classification_library_2) %>%
  mutate(ident = case_when(
...
per_sample_sbo_classifications
ggsave(plot = per_sample_sbo_classifications, filename = paste(results_dir, "Figure S2.png", sep = ""), width = 7, height = 5, device = "png", dpi = 700)
```
...
```{r include=FALSE}
elbow_plot <- ElbowPlot(cho.combined.sct,ndims = 100)

elbow_plot <- elbow_plot$data %>%
...
ggsave(plot = elbow_plot, filename = paste(results_dir, "Figure S3.png", sep = ""), width = 5, height = 5, device = "png", dpi = 700)
```


```{r include=FALSE}
cho.combined.sct <- RunUMAP(object = cho.combined.sct, dims = 1:10)
```
...
figure_s4a <- seurat_dim_plot_lib$data %>%
...
figure_s4b <- seurat_dim_plot_cc$data %>%
...
figure_s4 <- (figure_s4a + figure_s4b) 
figure_s4
ggsave(plot = figure_s4, filename = paste(results_dir, "Figure S4.png", sep = ""), width = 6, height = 2.5, device = "png", dpi = 700)
```
