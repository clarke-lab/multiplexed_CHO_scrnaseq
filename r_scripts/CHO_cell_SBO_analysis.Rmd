...
tm_qpcr_plot <- qpcr_data %>%
 ggboxplot( x = "Condition_edit", y = "fold_change",
               fill = "Condition_edit",palette = col_pal_vidiris) + 
  geom_point(data = qpcr_data, aes(x = Condition_edit, y = fold_change), 
...
tm_qpcr_plot 
ggsave(plot = tm_qpcr_plot, filename = paste(results_dir, "Figure 1A.png", sep = ""), width = 4.2, height =4, device = "png", dpi = 700)
```
...
cell_library_1 <- subset(cell_library_1, 
                         subset = nFeature_RNA > genes_detected_lower & 
                                  nFeature_RNA < genes_detected_upper & 
...
ggsave(plot = preprocessing_violin, filename = paste(results_dir, "Figure S1.png", sep = ""), width = 9, height = 7, device = "png", dpi = 700)
```
...
# Dimensionality reduction and clustering
merged_cell_obj <- RunPCA(object = merged_cell_obj, verbose = FALSE)
merged_cell_obj  <- FindNeighbors(merged_cell_obj , dims = 1:9)
merged_cell_obj  <- FindClusters(merged_cell_obj , resolution = 0.5)
merged_cell_obj <- RunUMAP(object = merged_cell_obj, dims = 1:25)
```

### Supplementary Figure 2
```{r fig.height=5, fig.width=8}
# overlay clusters
seurat_cluster_plot <- DimPlot(merged_cell_obj, reduction = "umap", group.by = c("seurat_clusters"),pt.size = 0.5) 
# overlay library
seurat_cluster_plot_lib <- DimPlot(merged_cell_obj, reduction = "umap", group.by = c("library"),pt.size = 0.5) 
# overlay cell cycle phase
seurat_cluster_plot_cc <- DimPlot(merged_cell_obj, reduction = "umap", group.by = c("Phase"),pt.size = 0.5)

cbp1 <- c("#E69F00", "#009E73",
          "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

figure_s2a <- seurat_cluster_plot_lib$data %>%
...
figure_s2b <- seurat_cluster_plot_cc$data %>%
...
figure_s2c <- seurat_cluster_plot$data %>%
...
figure_s2d <- seurat_cluster_plot$data %>%
...
(figure_s2a + figure_s2b) / (figure_s2c + figure_s2d)
```
...
tagged_cells_seurat_matrix1 <- as.matrix(GetAssayData(object = cell_library_1, slot = "counts"))
...
cho_1.hashtag[["percent.mt"]] <- cell_library_1$percent.mt
...
# cell_library_1 <- subset(cho_1.hashtag, subset = nFeature_RNA > 6000 & nFeature_RNA < 9000 &
...
tagged_cells_seurat_matrix2 <- as.matrix(GetAssayData(object = cell_library_2, slot = "counts"))
...
cho_2.hashtag[["percent.mt"]] <- cell_library_2$percent.mt
...
cell_library_2 <- subset(cho_2.hashtag, subset = nFeature_RNA > 6000 & nFeature_RNA < 9000 &
  nCount_RNA < count_high & nCount_RNA > count_low &
  percent.mt < mt_threshold)
...
cho.combined.sct <- RunUMAP(object = cho.combined.sct, dims = 1:25)
