...
```{r preparation, echo=T, results='hide'}
#load packages
package_list <- c(
...
Prior to running single cell analysis, qPCR was carried was to confirm that the expression of 3 genes and a spliced isoform of XBP1 were upregulated following tunicamycin addition
...
Load the data from GEM well 1 and GEM well 2
```{r load_rna_data}
# GEM well 1 
...
Show the knee plots and filtering of empty droplets. Function available from the Kallisto | Bustools [tutorial](<https://www.kallistobus.tools/tutorials/kb_getting_started/r/kb_intro_2_r/>)
```{r knee_plot, fig.height=4, fig.width=8}
...
# save
ggsave(
  filename = paste(results_dir, "Figure S1.png", sep = ""),
...
```{r mito_gene_list, include=FALSE}
...
```{r preprocess_rna_1, warning=FALSE}
...
```{r plot_preprocessing_output, fig.height=8, fig.width=10}
(
