...
#
 bc_rank1 <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/library1_bc_rank_current_pc.rds")
...
cell_library_1 <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/tagged_cells_lib1_current_pc.rds")
...
# knee_plot(bc_rank)
 ...
 bc_rank2 <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/library2_bc_rank_current_pc.rds")
 options(repr.plot.width=9, repr.plot.height=6)
...
cell_library_2 <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/tagged_cells_lib2_current_pc.rds")

cell_library_2
```
## Figure S1 
```{r fig.width=3, fig.width=7}
figure_s1 <- knee_plot_lib1 + knee_plot_lib2 + plot_annotation(tag_levels = 'a')
figure_s1
ggsave(plot = figure_s1, filename = paste(results_dir, "Figure S1.png", sep = ""), width = 12, height = 5, device = "png", dpi = 700)
```
...
figure_s2 
ggsave(plot = figure_s2, filename = paste(results_dir, "Figure S2.png", sep = ""), width = 7, height = 5, device = "png", dpi = 700)
```
...
sbo_lib_1.data <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/sbo_lib_1.data_current_pc.rds")
...
sbo_lib_2.data <- readRDS("~/GitHub/multiplexed_CHO_scrnaseq/data/new_rds/sbo_lib_2.data_current_pc.rds")
...
metacell_subset <- RunUMAP(metacell_subset, reduction = "pca", dims = 1:8)
...
```{r}
 geneInfo %>%
   filter(Initially.Assigned.Module.Color == "green") %>%
   arrange(-kMEgreen)
```
...
wgcna_boxplot_filtered <- plot_df %>%
  filter(variable %in% c("MEturquoise", "MEblue", "MEgreen")) %>%
  mutate(new=factor(variable,labels=c("MEturquoise","MEblue","MEgreen"),  levels=c("MEturquoise","MEblue","MEgreen"))) %>%
  ggplot(aes(x = orig.ident, y = value, fill = orig.ident)) +
  scale_fill_manual(values = c("#fde725", "#5ec962", "#21918c", "#31688e", "#440154")) +
...
selected_colors <- unique(geneInfo_annotated$Initially.Assigned.Module.Color)
